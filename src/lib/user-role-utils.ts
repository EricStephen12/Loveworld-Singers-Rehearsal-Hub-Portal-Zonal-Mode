...
/**
 * Check if user is a Boss (has BOSS prefix in email or role)
 */
export function isBoss(user: UserProfile | null): boolean {
  if (!user) return false
  
  // Check if user has boss role or email starts with "BOSS"
  return user.role === 'boss' || user.email?.toLowerCase().startsWith('boss')
}
