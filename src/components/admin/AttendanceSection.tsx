...
import { Calendar, Search, CheckCircle, Clock, XCircle, FileSpreadsheet, Download, RefreshCw, Users, TrendingUp, Plus, Printer, LogOut, AlertCircle, Check, AlertTriangle, UserCheck, UserX } from 'lucide-react'
...
export default function AttendanceSection() {
  ...
  const handleManualClockOut = async (recordId: string) => {
    try {
      await BackendAPI.generic.update('attendance', recordId, {
        check_out_time: new Date().toISOString()
      })
      // Update local state instantly
      setAttendanceRecords(prev => prev.map(r => r.id === recordId ? { ...r, check_out_time: new Date().toISOString() } : r))
    } catch (error) {
  ...
  const filteredRecords = attendanceRecords.filter((record) => {
    ...
    if (filterStatus === 'active') return record.check_in_time && !record.check_out_time
    if (filterStatus === 'completed') return !!record.check_out_time
    return true
  })
  ...
  return (
    <div className="flex-1 overflow-y-auto overflow-x-hidden bg-slate-50 pb-24 lg:pb-8 print:bg-white print:pb-0">
      <div className="max-w-7xl mx-auto px-4 lg:px-8 pt-5 lg:pt-8 print:px-0 print:pt-0">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4 mb-6 flex flex-col lg:flex-row gap-4 items-center justify-between print:hidden">
          {/* Segmented Filter Tabs */}
          <div className="flex items-center gap-1.5 p-1.5 bg-slate-50 rounded-2xl border border-slate-100 w-full lg:w-auto overflow-x-auto scrollbar-hide">
            {[
              { id: 'all', label: 'All Records', count: totalCount },
              { id: 'active', label: 'Active Now', count: presentCount },
              { id: 'completed', label: 'Clocked Out', count: clockedOutCount }
            ].map(tab => (
              <button
                key={tab.id}
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <div className="relative flex-1 lg:w-64">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 bg-slate-50 hover:bg-slate-100/50 transition-all font-medium"
              />
            </div>
            <button
              onClick={() => {
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 text-xs font-bold rounded-2xl border border-slate-200 transition-colors active:scale-95 flex-shrink-0"
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full p-8 border border-slate-100 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleManualCheckIn} className="space-y-5">
              ...
              <div>
                <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">Event Name</label>
                <input
                  type="text"
                  required
