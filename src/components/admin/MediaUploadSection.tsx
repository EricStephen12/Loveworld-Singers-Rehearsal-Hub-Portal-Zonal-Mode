  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row h-full bg-[#f8fafc] overflow-hidden">
      ...
      <MediaSidebar
        ...
        channelCount={mu.channels.length}
      />
      ...
      <div className="flex-1 flex flex-col h-full overflow-y-auto custom-scrollbar relative">
        ...
        {/* Main Content Area */}
        {['channels', 'channel-detail', 'videos', 'playlists', 'categories'].includes(mu.view) ? (
          <MediaContentList
            type={mu.view as any}
            videos={mu.videos}
            ...
            selectedChannel={mu.selectedChannel}
            setSelectedChannel={mu.setSelectedChannel}
            setView={mu.setView}
            ...
            zoneColor={mu.zoneColor}
            isHQAdmin={mu.isHQAdmin}
            onAdd={() => {
              if (mu.view === 'channels') { mu.resetChannelForm(); mu.setView('add-channel'); }
              else if (mu.view === 'videos') { mu.resetVideoForm(); mu.setView('add-video'); }
            ...
            onEdit={(item) => {
              if (mu.view === 'channels') mu.handleEditChannel(item);
              else if (mu.view === 'videos') mu.handleEditVideo(item);
          ...
          <MediaForms
            ...
            channelForm={mu.channelForm}
            setChannelForm={mu.setChannelForm}
            ...
            onSaveChannel={mu.handleSaveChannel}
        ...
        <MediaModals
          ...
          onConfirmDelete={(id) => {
            if (mu.deleteConfirm?.type === 'channel') mu.handleDeleteChannel(id);
            else if (mu.deleteConfirm?.type === 'video') mu.handleDeleteVideo(id);
