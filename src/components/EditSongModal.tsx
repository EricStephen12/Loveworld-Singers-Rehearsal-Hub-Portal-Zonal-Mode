...
export default function EditSongModal({
  ...
  const handleMediaFileSelect = (mediaFile: any) => {
    ...
    if (selectingPart === 'comment-audio') {
      ...
      // Selecting for main audio file (backward compatibility)
  ...
  // Handler functions for multiple audio parts
  ...
  useEffect(() => {
    if (song) {
      ...
      // Initialize audio parts (for AudioLab - vocal parts only)
      if (song.audioUrls) {
        // Use audioUrls directly (vocal parts: S/A/T/B + custom)
  ...
  const handleUpdate = async () => {
    if (songTitle.trim()) {
      ...
      const songData: PraiseNightSong = {
        ...
        // Multiple audio parts for AudioLab (vocal parts only: S/A/T/B + custom)
        audioUrls: audioUrls,
        customParts: customParts,
        availableParts: Object.keys(audioUrls).filter(k => audioUrls[k]), // Only include parts with URLs
