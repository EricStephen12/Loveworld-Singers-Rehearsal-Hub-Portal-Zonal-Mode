...
function PraiseNightPageContent() {
 ...
 console.error('Image failed to load:', ecardSrc);
      ...
      {(() => {
        const activeSongs = finalSongData.filter((song: any) => song.isActive);
        
        if (activeSongs.length > 0 && !isSongDetailOpen) {
          if (activeSongs.length === 1) {
            // Single active song - works like before
            ...
            return (
              ...
              >
                <div className="bg-white px-3 py-2.5 rounded-xl shadow-xl ring-2 ring-green-500 shadow-green-200/50 flex items-center gap-2.5 animate-pulse-ring max-w-[220px] sm:max-w-[260px]">
                  <div className="w-7 h-7 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
                    <AudioWave className="w-3.5 h-3.5 text-green-600" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[10px] font-bold text-green-600 uppercase tracking-widest mb-0.5 leading-none">Live Now</div>
                    <div className="text-xs font-semibold text-slate-900 truncate leading-tight">
                      {activeSongDisplay.title}
                    </div>
                  </div>
                  <div className="w-7 h-7 rounded-full bg-green-50 flex items-center justify-center group-hover:bg-green-100 transition-colors flex-shrink-0">
                    <Play className="w-3 h-3 text-green-600 ml-0.5" />
                  </div>
                </div>
              </div>
            );
          } else {
            // Multiple active songs - show menu
            return (
              <div className="fixed bottom-24 right-4 z-50 flex flex-col items-end">
                {/* Expanding Menu */}
                {showActiveMenu && (
                  <div className="mb-3 bg-white rounded-xl shadow-2xl border border-green-100 overflow-hidden w-64 animate-slide-up">
                    <div className="bg-green-50 px-3 py-2 border-b border-green-100 flex justify-between items-center">
                      <span className="text-xs font-bold text-green-700">LIVE SONGS</span>
                      <button onClick={() => setShowActiveMenu(false)} className="p-1 hover:bg-green-200 rounded-full transition-colors">
                    ...
                    <div className="max-h-[40vh] overflow-y-auto">
                      {activeSongs.map((song: any, i: number) => (
                        <button
                          key={song.id}
                          className="w-full text-left px-3 py-2.5 hover:bg-green-50 border-b border-gray-50 last:border-0 flex items-center gap-2 transition-colors"
                          onClick={() => {
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium text-slate-900 truncate">{song.title}</div>
                            <div className="text-[10px] text-slate-500 truncate">{song.category}</div>
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                >
                  <div className={`bg-white px-3 py-2.5 rounded-xl shadow-xl ring-2 ring-green-500 flex items-center gap-2.5 transition-all duration-300 ${!showActiveMenu ? 'animate-pulse-ring shadow-green-200/50' : 'bg-green-50'}`}>
                    <div className="w-7 h-7 rounded-full bg-green-500 flex items-center justify-center flex-shrink-0 shadow-inner">
                      <span className="text-white text-xs font-bold">{activeSongs.length}</span>
                    </div>
                    ...
                    <ChevronUp className={`w-4 h-4 text-green-600 transition-transform duration-300 ${showActiveMenu ? 'rotate-180' : ''}`} />
                  </div>
                </div>
              </div>
            );
