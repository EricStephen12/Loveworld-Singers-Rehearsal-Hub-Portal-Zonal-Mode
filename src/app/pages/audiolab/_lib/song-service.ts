...
export async function getSongs(zoneId?: string, limitCount: number = 100): Promise<AudioLabSong[]> {
  try {
    ...
    const songsWithAudio = songs.filter(song => 
      song.audioUrls && (
        song.audioUrls.full || 
        song.audioUrls.soprano || 
        song.audioUrls.alto || 
        song.audioUrls.tenor || 
        song.audioUrls.bass
      )
    );
...
function masterSongToAudioLabSong(doc: any): AudioLabSong {
  ...
  // Determine available parts from audioUrls
...
function determineAvailableParts(audioUrls: AudioUrls): VocalPart[] {
  ...
  if (audioUrls.full) parts.push('full');
  if (audioUrls.soprano) parts.push('soprano');
  if (audioUrls.alto) parts.push('alto');
  if (audioUrls.tenor) parts.push('tenor');
  if (audioUrls.bass) parts.push('bass');
