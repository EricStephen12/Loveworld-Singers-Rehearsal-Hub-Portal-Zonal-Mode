...
/**
 * Generate synced lyrics using AssemblyAI
 * This will transcribe the audio and return word-level timestamps
 */
export async function generateSyncedLyrics(
  ...
  try {
    const response = await fetch('/api/lyrics-sync', {
      ...
      body: JSON.stringify({ audioUrl, songId }),
    });
  ...
  try {
    ...
    if (audiolabDoc.exists()) {
      ...
      return {
        ...
        lyricsText: data.lyricsText || null,
        hasSyncedLyrics: !!(data.lyrics && data.lyrics.length > 0),
      };
    }
