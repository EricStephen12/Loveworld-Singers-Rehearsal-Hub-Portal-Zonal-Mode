...
export function ProjectSettingsSheet({
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div className="fixed bottom-0 left-0 right-0 z-[200] bg-[#1a1225] rounded-t-3xl animate-in slide-in-from-bottom duration-300 max-h-[85vh] overflow-y-auto">
        {/* Handle */}
        ...
        <div className="p-5 space-y-3 pb-10">
          {/* Project Name */}
          {isRenaming ? (
            <div className="flex items-center gap-2 p-3 bg-[#2a1e36] rounded-xl border border-violet-500/30">
              <input
                type="text"
                value={newName}
              ...
              <button
                ...
                disabled={isSaving}
                className="size-8 rounded-lg bg-violet-500 flex items-center justify-center text-white"
              >
            ...
            <button
              ...
              className="w-full flex items-center gap-4 p-4 bg-[#2a1e36] rounded-xl border border-white/5 hover:bg-white/5 transition-colors text-left"
            >
          ...
          {showInvite ? (
            <div className="p-4 bg-[#2a1e36] rounded-xl border border-violet-500/30 space-y-3">
              ...
              {/* Search Input */}
              <div className="flex items-center gap-2 p-2 bg-[#1f1429] rounded-lg border border-white/10">
                  ...
                  {searchResults.map(member => (
                    <div key={member.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5">
                      {member.avatar ? (
                        ...
                        <div className="size-8 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-400 text-sm font-bold">
                          {member.name[0]}
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className="px-3 py-1 bg-violet-500 text-white text-xs font-bold rounded-lg hover:bg-violet-600 disabled:opacity-50"
                      >
                ...
                <div className="pt-3 border-t border-white/10">
                  ...
                  <div className="space-y-2">
                    {collaboratorDetails.map(collab => (
                      <div key={collab.id} className="flex items-center gap-3 p-2 rounded-lg bg-white/5">
                        {collab.avatar ? (
              ...
              {collaboratorDetails.length > 0 && (
                <div className="flex -space-x-2">
                  {collaboratorDetails.slice(0, 3).map(c => (
                    c.avatar ? (
                      <img key={c.id} src={c.avatar} alt="" className="size-6 rounded-full border-2 border-[#2a1e36] object-cover" />
                    ) : (
                      <div key={c.id} className="size-6 rounded-full border-2 border-[#2a1e36] bg-violet-500/20 flex items-center justify-center text-violet-400 text-[10px] font-bold">
                        {c.name[0]}
                      </div>
                    )
                  ))}
                  {collaboratorDetails.length > 3 && (
                    <div className="size-6 rounded-full border-2 border-[#2a1e36] bg-white/10 flex items-center justify-center text-white text-[10px] font-bold">
                      +{collaboratorDetails.length - 3}
                    </div>
                  )}
                </div>
          ...
          {showDeleteConfirm ? (
            ...
            <button
              ...
              className="w-full flex items-center gap-4 p-4 bg-[#2a1e36] rounded-xl border border-white/5 hover:bg-red-500/10 hover:border-red-500/20 transition-colors text-left group"
            >
