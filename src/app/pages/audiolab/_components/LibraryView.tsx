...
export function LibraryView() {
  ...
  useEffect(() => {
    ...
    if (songParam && songs.length > 0 && handledSongParamRef.current !== songParam) {
      ...
      if (song) {
        ...
        if (!searchQuery) setSearchQuery(song.title);

        const timerId = setTimeout(() => {
          const element = document.getElementById(`song-${song.id}`);
          if (element) element.scrollIntoView({ behavior: 'smooth', block: 'center' });
          setTimeout(() => setIsNavigatingFromDetail(false), 800);
        }, 500);
        return () => clearTimeout(timerId);
      } else {
        // [DEEP SEARCH] Try fetching by ID specifically if not found in list
        const fetchDeep = async () => {
          try {
            const { getSongById } = await import('../_lib/song-service');
            const deepSong = await getSongById(songParam);
            if (deepSong) {
              const legacyDeep = (await import('../_lib/song-service')).toLeagcySong(deepSong);
              setSongs(prev => [legacyDeep, ...prev]);
              handledSongParamRef.current = songParam;
            }
          } catch (err) {
  ...
  const loadSongs = async (forceRefresh = false) => {
    try {
      ...
      // Show Load More if we got precisely the limit (indicating there might be more)
      setHasMore(legacySongs.length >= 450);
  ...
  // Load more songs (fetch next batch instead of refetching all)
  const loadMoreSongs = async () => {
    if (isLoadingMore || !hasMore) return;

    setIsLoadingMore(true);
    try {
      // Fetch a larger batch since we don't have true cursor pagination yet
      // but we append only new items to avoid displacement
      const nextLimit = songs.length + 500;
      const audioLabSongs = await getSongs(currentZone?.id, nextLimit);
      const legacySongs = audioLabSongs.map(toLeagcySong);

      // Filter out songs we already have
      const existingIds = new Set(songs.map(s => s.id));
      const newSongs = legacySongs.filter(s => !existingIds.has(s.id));

      if (newSongs.length === 0) {
        setHasMore(false);
      } else {
        setSongs(prev => [...prev, ...newSongs]);
        setHasMore(legacySongs.length >= nextLimit - 50);
      }
  ...
  const handleToggleExpand = useCallback((songId: string) => {
    setExpandedSongId(prev => {
      ...
      if (prev === songId && currentSong?.id === songId) {
        pause();
      }
  ...
  }, [currentSong, playSong, switchPart]);
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 py-1">
          ...
          >
            {isLoading ? (
              ...
              <RefreshCw size={18} className={`sm:w-5 sm:h-5 ${isLoading ? 'animate-spin' : ''}`} />
            )}
          </button>
        ...
        {!isLoading && filteredSongs.length > 0 && (
          <div className="flex-1 flex flex-col gap-2">
            {filteredSongs.map((song) => {
              ...
              const isThisSongLoading = currentSong?.id === song.id && state.player.isBufferLoading;
              ...
              return (
                <div key={song.id} id={`song-${song.id}`}>
                  <CollapsibleSongCard
                    ...
                    currentPart={isThisSongPlaying ? state.player.currentPart : null}
                    isBufferLoading={isThisSongLoading}
                    loadingTarget={state.player.loadingTarget}
