  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#191022] flex flex-col overflow-hidden text-white">
      ...
      <header className="relative z-20 flex items-center justify-between p-4 pt-6">
        <button 
          onClick={handleClose}
          className="flex size-10 items-center justify-center rounded-full bg-white/5 hover:bg-white/10 transition backdrop-blur-sm"
        >
          <X size={20} />
        </button>
        <div className="flex flex-col items-center flex-1 mx-4">
          <h2 className="text-lg font-bold leading-tight tracking-tight text-center truncate w-full max-w-[200px]">
            {songTitle}
          </h2>
          <p className="text-white/60 text-xs font-medium tracking-wide">{songArtist}</p>
        </div>
      ...
      {hasSong && (
        ...
        >
          ...
          <div className="w-full px-4 py-6 relative">
            {/* Waveform Background */}
            <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-20 flex items-center justify-center gap-1 opacity-30 pointer-events-none">
              {waveformBars.map((height, i) => (
                <div 
                  key={i}
                  className="w-2 bg-violet-500 rounded-full transition-all duration-150"
            ...
            {/* Lyric Text */}
            <h1 
              className="text-white text-[32px] md:text-[40px] font-bold leading-tight text-center relative z-10"
      ...
      {hasSong && (
        <div className="relative z-30 pb-8 pt-2 px-6 w-full bg-gradient-to-t from-[#191022] via-[#191022] to-transparent">
          <div className="bg-[#2a1d35]/90 backdrop-blur-xl border border-white/10 rounded-[2rem] p-5 shadow-2xl max-w-md mx-auto">
            ...
            {/* Buttons */}
            <div className="flex items-center justify-between">
              <button className="size-10 flex items-center justify-center rounded-full text-white/50 hover:text-white hover:bg-white/10 transition">
                <Maximize size={24} />
              </button>
              ...
              <div className="flex items-center gap-4">
                <button 
                  onClick={skipBack}
                  className="size-12 flex items-center justify-center rounded-full text-white hover:text-violet-400 hover:bg-white/5 transition active:scale-95"
                >
                  <RotateCcw size={26} />
                </button>
                <button 
                  onClick={handlePlayPause}
                  className="size-16 flex items-center justify-center rounded-full bg-violet-500 text-white shadow-lg shadow-violet-500/40 hover:bg-violet-600 active:scale-90 transition"
                >
                  {player.isPlaying ? (
              ...
              <button 
                onClick={toggleMic}
                className={`size-10 flex items-center justify-center rounded-full transition ${
                  isMicActive 
