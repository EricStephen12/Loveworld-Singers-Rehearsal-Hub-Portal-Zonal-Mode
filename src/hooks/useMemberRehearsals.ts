/**
 * Hook for getting combined rehearsals (zone + sub-group) for a member
 * Phase 4: Sub-Group Rehearsals
 */
...
export function useMemberRehearsals(): UseMemberRehearsalsReturn {
  ...
  // Get filtered rehearsals based on current filter
  const getFilteredRehearsals = useCallback(() => {
    switch (filter) {
      case 'zone':
        return zoneRehearsals;
      case 'subgroup':
        return subGroupRehearsals;
      default:
        return rehearsals;
    }
