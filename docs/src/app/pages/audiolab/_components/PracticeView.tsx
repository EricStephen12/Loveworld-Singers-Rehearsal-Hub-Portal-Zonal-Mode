...
import { Mic, Activity, Music2, Dumbbell, Flame, Zap, Clock, ListChecks, GraduationCap, Award, Bell, Loader2, Play } from 'lucide-react';
    ...
    <div className="relative pb-24">
            ...
            <div className="flex gap-3 sm:gap-4">
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-1">
                  <Flame size={12} className="sm:w-[14px] sm:h-[14px] text-orange-400" />
      ...
      {state.player.currentSong && (
        <section className="relative z-10 px-3 sm:px-4 mt-4 sm:mt-6">
          <h2 className="text-white text-lg sm:text-xl font-bold tracking-tight mb-3 sm:mb-4">Quick Practice</h2>
          <button
            onClick={() => setView('karaoke')}
            className="w-full flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700 text-white transition-all shadow-lg shadow-violet-500/30 active:scale-[0.98] touch-manipulation"
          >
            <div className="size-10 sm:size-12 rounded-full bg-white/20 flex items-center justify-center flex-shrink-0">
              <Play size={20} className="sm:w-6 sm:h-6" fill="currentColor" />
            </div>
            <div className="flex-1 text-left min-w-0">
              <p className="text-[10px] sm:text-xs font-bold text-white/80 uppercase tracking-wider mb-0.5">
                Continue
              </p>
          ...
          {practiceCards.map((card) => {
            ...
            return (
              <button
                ...
                className={`group relative flex flex-col justify-between overflow-hidden rounded-xl bg-[#261933] p-3 sm:p-4 aspect-[5/4] border shadow-md transition-all duration-300 active:scale-95 cursor-pointer text-left touch-manipulation ${
                  card.available 
